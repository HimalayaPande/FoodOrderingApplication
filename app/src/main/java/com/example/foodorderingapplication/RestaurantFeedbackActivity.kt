...
class RestaurantFeedbackActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val restid=intent.getStringExtra("restid4").toString()
        val recyclervieww= findViewById<RecyclerView>(R.id.restaurantfeedback)
        ...
        feedback_database?.addValueEventListener(object : ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                for(items in snapshot.children){
                    if(restid==items.child("rest_id").getValue()){
